[Open the Data Notebook in Colab](https://colab.research.google.com/github/lorinctoldi/deep-learning/blob/main/data.ipynb)

[![Open in Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/lorinctoldi/deep-learning/blob/main/data.ipynb)

[Open the Model Notebook in Colab](https://colab.research.google.com/github/lorinctoldi/deep-learning/blob/main/model.ipynb)
...
**Note**: if you are looking for our final and detailed IEEE formatted documentation, you may find the final pdf in the `/documentation` folder!

# Goal in the course
We are aiming for the offered grade (megajánlott jegy).
...
- `documentation/` - Project documentation and written materials.
- `presentation/` - Final project presentation files.
- `experiments/` - Contains all models that were experimented with during this project.
- `experiments/{model_name}/` - Contains all files that are required for the given model. (`utils.py`, `contants.py`, `model.py`)
- `utils.py` - Contains utility functions used in the notebook.
- `constants.py` - Contains constants values used in the notebook.
- `data.ipynb` - Contains all logic for data analysis, and description.
- `model.ipynb` - Contains all logic for the final U-Net model.
- `requirements.txt` - Lists all required Python packages.

> **Important Note:**
> The `data/` folder is initially empty.
> The dataset download and setup methods are included in the `data.ipynb` notebook.
...
Open data.ipynb and run all cells to download the dataset, clean masks, and perform exploratory analysis.
...
- we asked for help in getting inspiration for training parameters
- sometimes we used it for bug search, checking syntactical correctness
- used it for code review to spot bugs early on
